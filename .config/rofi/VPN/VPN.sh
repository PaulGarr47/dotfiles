...
# Paths
main_theme="~/.config/rofi/powermenu/style.rasi"
sub_theme="~/.config/rofi/config.rasi"
...
rofi_menu() {
	echo -e "$connect\n$disconnect\n$switch\n$import\n$quit" | \
	rofi -dmenu -p "VPN Menu" -theme "$main_theme" -l 5
}

connect_cmd() {
	local choice
	choice=$(basename -s .conf $(sudo ls $wg_path) | rofi -dmenu -p "Select Connection" -theme "$sub_theme")
...
disconnect_cmd() {
	local active
	active=$(sudo wg 2>&1 | grep interface | awk '{print $2}' | sed 's/://g' | rofi -dmenu -p  "Disconnect Selection" -theme "$sub_theme")
...
switch_cmd() {
	...
	rofi -dmenu -p  "Select current connection to disconnect" -theme "$sub_theme")
	new=$(basename -s .conf $(ls $wg_path) | rofi -dmenu -p "Select Connection" -theme "$sub_theme")
...
import() {
	local selected
	selected=$(find -d 2 -t file -e conf . | rofi -dmenu -p "Select to copy" -multi-select -mesg "Shift + Enter to select multiple" -theme "$sub_theme")
